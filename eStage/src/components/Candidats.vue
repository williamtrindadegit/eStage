<template>
  <section class="m-5 p-10 bg-gray-100 rounded-lg">
    ...
    <router-link to="/addcandidate" class="mt-20 btn btn-ghost text-md m-1 p-2 border-solid border-2 text-white bg-red-500 mx-2">
    ...
    <div class="bg-white mx-2 mt-4 mb-16 p-8 rounded-lg">
      <div class="grid grid-cols-3 auto-cols-max">
        <div v-for="candidat in candidats" :key="candidat._id" class="max-w-sm rounded overflow-hidden shadow-lg m-2">
          <div class="px-6 py-4">
            ...
            <p>Compétences : {{ candidat.skills.join(', ') }}</p>
          </div>
        </div>
      </div>
    </div>
