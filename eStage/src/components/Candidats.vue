<template>
  <section class="m-5 p-10 bg-gray-100 rounded-lg">
    ...
    <router-link to="/addcandidate" class="mt-20 btn btn-ghost text-md m-1 p-2 border-solid border-2 text-white bg-purple-700 mx-2">
    ...
    <div class=" mx-2 mt-4 mb-16 p-8 rounded-lg">
      <div class="grid sm:grid-cols-1 lg:grid-cols-2 auto-cols-max">
        <div v-for="candidat in candidats" :key="candidat._id" class="overflow-hidden shadow-lg m-2 ">
          <div class="px-6 py-4 flex flex-row bg-white hover:bg-slate-200 hover:m-1 cursor-pointer" @click="onDetails(candidat._id)">
            <div>
              <img className="w-24 h-24 p-0 m-0 rounded-md"
              src="https://api.dicebear.com/8.x/icons/svg?seed=Casper&backgroundColor=9fa8da&icon=emojiSmile"
              alt="avatar" />
