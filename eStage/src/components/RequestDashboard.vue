<template>
  <section className="m-5 p-10 bg-gray-100 rounded-lg">
    ...
    <div className="bg-white mx-2 mt-4 mb-16 p-8 rounded-lg">
      ...
      <!-- Grid -->
      <div className="mt-6 md:grid md:grid-cols-4 gap-4" v-for="demande in demandes" :key="demande._id">
        <div className="flex">
          ...
          <div className="text-black text-xs md:ml-2 my-auto">
            <h3 className="font-semibold">{{ demande.title }} </h3>
            <h4>{{ `${demande.candidate.firstName} ${demande.candidate.lastName}` }}</h4>
          </div>
        </div>
        <div className="text-black text-xs flex items-center">{{ demande.description }}</div>
        <div className="text-black text-xs flex items-center">{{ new Date(demande.startDate).toLocaleDateString('fr-CA')
          }}</div>
