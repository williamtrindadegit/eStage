...
onMounted(async () => {
    ...
    if (response > 0) {
        ...
        offres.value = await InternshipOffers.FindAll();// getData(3);
    }
...
function edit(offre) {
    ...
    let id = offreCurrent.value._id
    router.push({ name: "editinternshipoffer", params: { id: offreCurrent.value._id }})
}

function deleteMe(id){

    offres.value = InternshipOffers.Delete(id);   
}
